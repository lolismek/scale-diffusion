...
class SceneApp extends Scene {
  constructor() {
    super();
    // this.orbitalControl.lock(); // Disable mouse interaction

    this.orbitalControl.radius.value = 5.0; // Closer = bigger
    this.orbitalControl.radius.limit(4, 6);
    ...
    this._center = [0, 0, 0];
    // Disabled hit testing since controls are locked
    // if (!GL.isMobile) {
    //   this._initHit();
    // }
  ...
  update() {
    this._length = Math.sin(Scheduler.getTime() * 0.85) * 0.45 + 0.55;
    ...
    this._drawSim
      ...
      .uniform("uTime", Scheduler.getTime() + this._seedTime)
  ...
  _renderRibbon(mShadow = false) {
    ...
    this._drawRibbon
      ...
      .uniform("uTime", Scheduler.getTime())
  ...
  render() {
    ...
    // Skip background for transparency
