...
const vertexShader = `
  ...
  void main() {
    ...
    vec3 newNormal = normalize(cross(ptBitangentSample - p0, ptTangentSample - p0));
...
const fragmentShader = `
  ...
  void main() {
    ...
    vec3 col = mix(color1, color2, ss(2.0, 6.0, length(rPos)));

    // Lighter warm base
    vec3 baseColor = vec3(0.20, 0.14, 0.12);

    // Isolines
    float coord = length(rPos) * 4.0;
    float line = abs(fract(coord - 0.5) - 0.5) / fwidth(coord) / 1.25;
    float grid = 1.0 - min(line, 1.0);

    // Brighter lines
    vec3 finalColor = mix(baseColor, col * 2.5, grid);
    finalColor = mix(finalColor, vec3(0.0), bloom);
    finalColor = mix(finalColor, col * 3.0, grid);
...
export function Icosahedron({ className, style }) {
  ...
  useEffect(() => {
    ...
    // Icosahedron with higher detail like original
    ...
    const material = new THREE.ShaderMaterial({
      uniforms: {
        ...
        color2: { value: new THREE.Color(0xe8a088) }
      },
      vertexShader,
      fragmentShader,
      transparent: true
    ...
    sceneRef.current = { scene, camera, renderer, material, mesh, bloomComposer, finalComposer, globalUniforms }
    ...
    const animate = () => {
      ...
      // Render bloom pass (dark base, bright lines)
      globalUniforms.bloom.value = 1
      bloomComposer.render()

      // Render final pass (normal + bloom)
